...
def main(args):
    ...
    if not os.path.exists(dstdir):
        os.mkdir(dstdir)
    #label记录txt
    ftxt=os.path.join(data_dir,'widerface/train/label.txt')
    #记录label的txt
    f=open(os.path.join(OUTPUT,'landmark_%d_aug.txt'%(size)),'w')
    #获取图像路径，box，关键点
    data=getDataFromTxt(ftxt)
    idx=0
    for (imgPath,boxes,landmarkGts) in tqdm(data):
        #存储人脸图片和关键点
        img=cv2.imread(imgPath)
        
        img_h,img_w,img_c=img.shape
        
        for idxs in range(len(landmarkGts)):
            F_imgs=[]
            F_landmarks=[]
            landmarkGt = landmarkGts[idxs]
            box = boxes[idxs]
