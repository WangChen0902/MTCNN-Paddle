...
def main(args):
    ...
    if size==12:
        ...
        save_size = 48
    #原图片地址
    base_dir = '../data/widerface/train'
    anno_file = '../data/widerface/train/label.txt'
    #处理后的图片存放地址
    ...
    PNetDetector = FcnDetector(pnet, model_path[0])
    detectors[0] = PNetDetector
    if net =='RNet':
        rnet = RNet(phase='test')
        RNetDetector = Detector(rnet, 24, batch_size[1], model_path[1], 'RNet')
        detectors[1] = RNetDetector

    #读取文件的image和box对应函数在utils中
    data = read_annotation(base_dir, anno_file)
